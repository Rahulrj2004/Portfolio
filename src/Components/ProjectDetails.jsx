...
const ProjectDetails = ({ title, description, subDescription, href, image, tags, closemodel }) => {
    const invertedIcons =[ "Flask", "SK learn"];
                ...
                <div className=" p-5">
                    ...
                    <div className="flex items-center justify-between mt-4">
                        <div className="flex gap-3">
                            {tags.map((tag) => (
                                <img key={tag.id} src={tag.path} alt={tag.name} className={`rounded-lg size-10 hover-animation ${invertedIcons.includes(tag.name) ? "invert": ""}`} />
                            ))}
                        </div>
